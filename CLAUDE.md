...
- **Strona serwerowa (RPi) do przebudowy od zera:** całość setupu Raspberry Pi (skrypty, broker, hostname/mDNS, konto brokera, autostart) ma być **zbudowana od nowa jako kompleksowy plug-and-play** (jedno polecenie, łącznie z setupem i podłączaniem). Obecne `scripts/` to wersja tymczasowa — nie traktuj jej jako docelowej architektury.
- **Luki obecnego setupu RPi** (zostaną rozwiązane przy przebudowie): `install.sh`/`host_config.sh` nie ustawiają hostname `rpi-smarthome` ani nie instalują `avahi-daemon` (wymagane przez mDNS w firmware), oraz nie tworzą pliku `mosquitto/config/passwd` (broker ma `allow_anonymous false`, firmware loguje się jako `esp32`/`esp32`). Tymczasowy obejście opisuje [GETTING_STARTED.md](GETTING_STARTED.md).
...
- **Bezpieczeństwo:** dane brokera (`esp32`/`esp32`) i PoP są zaszyte w kodzie — akceptowalne dla makiety, nie dla produkcji.
